...
class CNSConverter:
    ...
    def check_attributes_exist(self):
        for attr in self.attributes.values():
            self.__attr_existence[attr] = self.__refln_data.hasAttribute(attr)
        
        self.check_Io_attribute_existence()

    def check_Io_attribute_existence(self):
        # Define the attributes you want to check
        attributes = ["intensity_meas", "intensity_meas_au", "intensity"]

        # Initialize __attr_existence dictionary
        self.__attr_existence["Io"] = False

        # Loop over each attribute to check if it exists
        for attr in attributes:
            if self.__refln_data.hasAttribute(attr):
                # If any attribute exists, set "Io" in __attr_existence to True and break the loop
                self.__attr_existence["Io"] = True
                break

    def check_sIo_attribute_existence(self):
        # Define the attributes you want to check
        attributes = ["intensity_sigma", "intensity_sigma_au", "intensity_sigm", "intensity_meas_sigma", "intensity_meas_sigma_au"]

        # Initialize __attr_existence dictionary for sIo
        self.__attr_existence["sIo"] = False

        # Loop over each attribute to check if it exists
        for attr in attributes:
            if self.__refln_data.hasAttribute(attr):
                # If any attribute exists, set "sIo" in __attr_existence to True and break the loop
                self.__attr_existence["sIo"] = True
                break

    def check_status_attribute_existence(self):
        # Define the attributes you want to check
        attributes = ["status", "R_free_flag", "statu", "status_au"]

        # Initialize __attr_existence dictionary for status
        self.__attr_existence["status"] = False

        # Loop over each attribute to check if it exists
        for attr in attributes:
            if self.__refln_data.hasAttribute(attr):
                # If any attribute exists, set "status" in __attr_existence to True and break the loop
                self.__attr_existence["status"] = True
                break
    ...
    def write_cns_file_at_index(self, test):
        """
        Method to write data to a CNS file
        """
        with open(self.__fout_path, 'w') as output_file:
            ...
            # Assuming that all the variables here are boolean values
            # self.initialize_Io_at_index(0)
            # self.initialize_sIo_at_index(0)
            # self.initialize_status_at_index(0)
            self.initialize_columns_at_index(0)
            ...
            for i in range(self.__nref):
                # Initialize the Io, sIo, status, and other variables at index i
                # self.initialize_Io_at_index(i)
                # self.initialize_sIo_at_index(i)
                # self.initialize_status_at_index(i)
                self.initialize_columns_at_index(i)

                h, k, l, ff, sff, ii, sii = self.new_get_F_I(i)

                if self.__status and self.__status == 'x':  #need to change here
                    continue
    ...
    def convert(self):
        ...
        self.write_cns_file_at_index(test=0)
...
def test_initialize_columns_at_index():
    sffile = SFFile()
    sffile.readFile(str(Path("/Users/vivek/Library/CloudStorage/OneDrive-RutgersUniversity/Desktop files/Summer/py-rcsb_apps_sfconvert/sf_convert_project/src/sf_convert/cif_files/5pny-sf.cif")))
    CNSexport = CNSConverter(sffile, str(Path("your_output_file.txt")))

    # import random
    # i = random.randint(0, 100)  # Assuming the range of valid indexes is from 0 to 100.
    # CNSexport.initialize_columns_at_index(i)

    CNSexport.convert()

    # Verify the results here.

#test_initialize_columns_at_index()
