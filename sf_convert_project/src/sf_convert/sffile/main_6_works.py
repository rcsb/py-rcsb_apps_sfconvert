...
def is_not_empty_line(string):
    return len(string) - string.count(' ')


def get_non_empty_lines(input_file):
    if not os.path.isfile(input_file):
        print(f"Cannot open file ({input_file}) in get_non_empty_lines")
        return None, 0

    with open(input_file, 'r') as fp:
        lines = [line for line in fp.readlines() if is_not_empty_line(line) > 0]

    return lines, len(lines)


def print_and_write(fout, message):
    print(message)
    fout.write(message + '\n')


def write_format_and_key(fout, format, key):
    length = len(format)
    if length < 1:
        print_and_write(fout, "Sorry! Format cannot be predicted. Check input file!")
    else:
        fout.write("FORMAT=%s\n" % format)
        write_key(fout, key)
    fout.close()
...
def write_key(fout, key):
    ...
    print_and_write(fout, keys.get(key, ""))
...
def guess_sf_format(input_file):
    lines, num_lines = get_non_empty_lines(input_file)
    if num_lines < 2:
...
def process_lines(lines, variables, fout):
    for i, line in enumerate(lines):
        if i == 0 and line[0:4] == 'MTZ ':
            ...
            write_format_and_key(fout, "MTZ", 10)
            return
...
def is_other_line(line):
    return (strncmp_case(line, "!SPACE_GROUP_NUMBER=", 20) == 0 or strncmp_case(line, "!UNIT_CELL_CONSTANTS=", 21) == 0 or
...
def determine_format(variables, fout, format_value):
    if variables["n1"] >= 4:
        if variables["k1"] == 1 and variables["k2"] == 1:
            write_format_and_key(fout, "MMCIF", 2)  # I&F
        elif variables["k1"] == 1 and variables["k2"] != 1:
            write_format_and_key(fout, "MMCIF", 1)  # F
        elif variables["k1"] != 1 and variables["k2"] == 1:
            write_format_and_key(fout, "MMCIF", 0)  # I
        elif variables["k3"] == 1:
            write_format_and_key(fout, "MMCIF", 3)  # ano
        else:
            write_format_and_key(fout, "MMCIF", -1)  # nothing

        format_value = "MMCIF"
    elif variables["n2"] >= 4:
        if variables["k1"] == 1 and variables["k2"] == 1:
            write_format_and_key(fout, "CIF", 2)  # I&F
        elif variables["k1"] == 1 and variables["k2"] != 1:
            write_format_and_key(fout, "CIF", 1)  # F
        elif variables["k1"] != 1 and variables["k2"] == 1:
            write_format_and_key(fout, "CIF", 0)  # I
        else:
            write_format_and_key(fout, "CIF", -1)  # nothing

        format_value = "CIF"
    elif variables["n3"] >= 50 and variables["mm1"] < 10:
        if variables["m1"] > 100 and variables["m2"] > 100:
            write_format_and_key(fout, "CNS", 2)  # I&F
        elif variables["m1"] > 50 and variables["m2"] < 10:
            write_format_and_key(fout, "CNS", 1)  # F
        elif variables["m2"] > 50 and variables["m1"] < 10:
            write_format_and_key(fout, "CNS", 0)  # I
        else:
            write_format_and_key(fout, "CNS", -1)  # nothing

        format_value = "CNS"
    elif variables["n3"] >= 50 and variables["mm1"] > 10:
        write_format_and_key(fout, "xplor", 1)
        format_value = "xplor"
    elif variables["n4"] >= 100:
        if variables["m2"] > 0:
            write_format_and_key(fout, "TNT", 0)
        else:
            write_format_and_key(fout, "TNT", -2)

        format_value = "TNT"
    elif variables["n5"] >= 5:
        write_format_and_key(fout, "XSCALE", 0)
        format_value = "XSCALE"
    elif variables["n6"] >= 6:
        write_format_and_key(fout, "DTREK", 0)
        format_value = "DTREK"
    elif variables["n7"] > 1 or (variables["nn7"] >= 200 and variables["nn7"] == variables["kk7"]):
        write_format_and_key(fout, "HKL", 0)
        format_value = "HKL"
    elif variables["n8"] >= 200:
        if variables["m2"] > 0:
            write_format_and_key(fout, "SHELX", 0)
        else:
            write_format_and_key(fout, "SHELX", -2)

        format_value = "SHELX"
    elif variables["n9"] >= 200:
        write_format_and_key(fout, "SAINT", 0)
        format_value = "SAINT"
    else:
        write_format_and_key(fout, "", -1)

    return format_value

guess_sf_format('input.txt')